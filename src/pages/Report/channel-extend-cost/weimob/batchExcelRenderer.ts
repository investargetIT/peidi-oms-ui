...
import type { WmBatchStatResult } from './batchStatBuilder';

/**
 * 微信 - 批量导出 Excel 渲染
 *
 * 每家店一个 Excel，含 2 个 Sheet，对应「费用统计」弹窗里的 2 张表：
 *   Sheet 1「微盟余额对账」 —— 单行 11 列（列与弹窗顶部汇总表一致）
 *   Sheet 2「微盟费用统计」 —— 费用分类（合并）/ 分类 / 业务类型 / 收入 / 支出 + 末尾「合计」行
 */
...
// ===== Sheet 1：微盟余额对账 =====
function renderBalanceSheet(workbook: ExcelJS.Workbook, result: WmBatchStatResult) {
  const ws = workbook.addWorksheet('微盟余额对账');
  const headers = [
    '账单月份',
    '平台',
    '账户名称',
...
// ===== Sheet 2：微盟费用统计 =====
function renderDetailSheet(workbook: ExcelJS.Workbook, result: WmBatchStatResult) {
  const ws = workbook.addWorksheet('微盟费用统计');
 ...
 */
export async function renderWmStatExcel(result: WmBatchStatResult): Promise<Blob> {
