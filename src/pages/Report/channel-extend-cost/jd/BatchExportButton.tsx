...
import { getMonthRange, buildJdStat } from './batchStatBuilder';
import { renderJdStatExcel } from './batchExcelRenderer';
...
/**
 * 京东 TAB - 导出当前选中月份的所有店铺统计费用
 *
 * 点击按钮 -> 拉取京东全部店铺 -> 每家店调 getJdCostStat（jd-cost-stat）
 * -> 按弹窗相同的口径算出三张表（余额对账 / 钱包支出分类 / 账单收支计算）
 * -> 生成 Excel -> 全部打包成 zip 下载。
 *
 * 失败处理：失败的店铺仍生成空模板，但会把店名 + 失败原因写入 zip 内的 _失败明细.txt。
 */
const BatchExportButton: React.FC<BatchExportButtonProps> = ({ yearMonth }) => {
  ...
  const fetchJdShops = async (): Promise<ShopVo[]> => {
  ...
  const doExport = async () => {
    ...
    await pMap(shops, 5, async (shop) => {
      ...
      const fileBase = safeName(`${shopName}_${yearMonth}_京东费用统计`);
      const shopId = shop.id as number;
      try {
        const res = await ChannelExtendCostApi.getJdCostStat({ shopId, startDate, endDate });
        if (res.code !== 200) {
          throw new Error('getJdCostStat 返回非 200');
        }
        const data = res.data || ({} as any);
        const statResult = buildJdStat({
          jd2ExpenseStat: data.jd2ExpenseStat || [],
          jd1CalculateStat: data.jd1CalculateStat || [],
          lastMonthJd1CalculateStat: data.lastMonthJd1CalculateStat || [],
          lastMonthEndingBalance: data.lastMonthEndingBalance ?? null,
          yearMonth,
          shopName,
        });
        const blob = await renderJdStatExcel(statResult);
        ...
        // 生成空模板，避免下载包里缺这家店
        try {
          const emptyStat = buildJdStat({
            jd2ExpenseStat: [],
            jd1CalculateStat: [],
            lastMonthJd1CalculateStat: [],
            lastMonthEndingBalance: null,
            yearMonth,
            shopName,
          });
          const blob = await renderJdStatExcel(emptyStat);
          zip.file(`${fileBase}.xlsx`, blob);
        } catch (e2: any) {
    ...
    const zipName = `京东全部统计费用_${yearMonth}_${dayjs().format('YYYYMMDD_HHmmss')}.zip`;
  ...
  return (
    <>
      ...
      <Modal
        title={`导出 京东 ${yearMonth} 全部店铺统计费用`}
        open={modalOpen}
        onCancel={closeModal}
        onOk={doExport}
      ...
      >
        {!exporting && !exportSummary && (
          <Space direction="vertical" size={8} style={{ width: '100%' }}>
            <div>
              将导出 <Text strong>{shops.length}</Text> 家 京东 店铺在{' '}
            ...
            {fetchingShops ? (
              ...
              <div style={{ color: '#ff4d4f' }}>
                <ExclamationCircleOutlined /> 未找到 京东 店铺，请确认店铺配置。
              </div>
            ) : (
              <div style={{ color: '#999', fontSize: 12, lineHeight: 1.7 }}>
                · 每店一个 Excel，含 3 个 Sheet：京东余额对账 + 京东钱包支出分类统计 +
                京东账单收支计算列表
                <br />
                · 样式尽量还原前端弹窗（加粗、底纹、校验列染色）
                <br />· 失败的店铺会生成空模板，错误信息写入 zip 内的 _失败明细.txt
              </div>
            )}
          </Space>
        )}
