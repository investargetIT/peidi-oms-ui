...
import type { TmallBatchStatResult } from './batchStatBuilder';

/**
 * 天猫 - 批量导出 Excel 渲染
 *
 * 每家店一个 Excel，含 2 个 Sheet，对应「费用统计」弹窗里的 2 张表：
 *   Sheet 1「天猫余额对账」 —— 单行 11 列（列与弹窗顶部汇总表一致）
...
// ===== Sheet 1：天猫余额对账 =====
function renderBalanceSheet(workbook: ExcelJS.Workbook, result: TmallBatchStatResult) {
  const ws = workbook.addWorksheet('天猫余额对账');
  const headers = [
    '账单月份',
    '平台',
    '账户名称',
...
/**
 * 把天猫统计结果渲染成 Excel Blob
 */
export async function renderTmallStatExcel(result: TmallBatchStatResult): Promise<Blob> {
