...
import type { KuaishouBatchStatResult } from './batchStatBuilder';

/**
 * 快手 - 批量导出 Excel 渲染
 *
 * 每家店一个 Excel，含 2 个 Sheet，对应「费用统计」弹窗里的 2 张表：
 *   Sheet 1「快手余额对账」 —— 单行 11 列（列与弹窗顶部汇总表一致）
 *   Sheet 2「快手费用统计」 —— 分类（合并单元格）/ 管报名称 / 收入 / 支出 + 末尾「合计」行
 */
...
// ===== Sheet 1：快手余额对账 =====
function renderBalanceSheet(workbook: ExcelJS.Workbook, result: KuaishouBatchStatResult) {
  const ws = workbook.addWorksheet('快手余额对账');
  const headers = [
    '账单月份',
    '平台',
    '账户名称',
...
// ===== Sheet 2：快手费用统计 =====
function renderDetailSheet(workbook: ExcelJS.Workbook, result: KuaishouBatchStatResult) {
  const ws = workbook.addWorksheet('快手费用统计');
...
/**
 * 把快手统计结果渲染成 Excel Blob
 */
export async function renderKuaishouStatExcel(result: KuaishouBatchStatResult): Promise<Blob> {
