...
export interface WxBalanceSummary {
  billMonth: string;
  accountName: string;
  platform: string;
  endBalance: number;
...
export interface WxBatchStatResult {
  /** 微信余额对账（顶部汇总表），income/expense/total 供合计行使用 */
  summary: WxBalanceSummary & { income: number; expense: number; total: number };
  /** 明细表行（按接口返回顺序平铺展示，不含总计行；分级字段由后端自带） */
  rows: WxDetailRow[];
  /** 总计行（businessType=总计） */
  totalRow: WxDetailRow | null;
 ...
 *   校验     = 计算余额 - 期末余额
 */
export function buildWxStat(args: {
  wxStatData: FinanceLedgerSummaryRowVo[];
  beginningBalance: number | null;
  endingBalance: number | null;
  yearMonth: string;
  shopName: string;
}): WxBatchStatResult {
  const { wxStatData, beginningBalance, endingBalance, yearMonth, shopName } = args;

  // —— 平铺流水行（总计行单独抽出），按接口返回顺序展示，不做额外排序/合并映射 ——
  let totalRow: WxDetailRow | null = null;
  const flat: WxDetailRow[] = [];
