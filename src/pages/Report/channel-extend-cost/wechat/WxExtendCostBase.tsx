...
export interface WxExtendCostBaseProps {
  channel: string;
...
const WxExtendCostBase: React.FC<WxExtendCostBaseProps> = ({
  channel,
  extraActions,
  onYearMonthChange,
  onCustomStatClick,
}) => {
  // 后端 channel 编码：微信=wx（其余=自身）。group/page、cost-category-stat 传编码；店铺搜索用中文
  const channelCode = channel === '微信' ? 'wx' : channel;
  ...
  const [wxStatData, setWxStatData] = useState<FinanceLedgerSummaryRowVo[]>([]);
  ...
  const fetchShops = async () => {
    ...
    try {
      const params: any = {
        sortStr: '',
        // 微信：channel = 微信，platform = 微信
        searchStr: JSON.stringify([
          { searchName: 'channel', searchType: 'like', searchValue: '微信' },
  ...
  const fetchChannelData = async (params: PageRequest = {}) => {
    ...
    try {
      const searchParams: any = {
        ...
        // channel 本地联调按后端排期传中文「微信」（原 wx/编码），后端调整后可改回 channelCode
        channel,
  ...
  const openStatModal = async (record: FinanceChannelExtendCostItemVo) => {
    ...
    const title = `${displayShopName(wdtName) || ''} ${yearMonth} 微信费用统计`;
    ...
    try {
      ...
      console.error('获取微信费用统计数据失败:', error);
      message.error('获取微信费用统计数据失败');
    } finally {
      setStatLoading(false);
    }
  };

  // 微信统计结果（余额对账 + 明细行），与批量导出共用 batchStatBuilder，保证口径一致。
  //   summary：本期收款=收入列之和、本期费用=支出列之和、期末/上月余额来自成本分类统计
  //   rows    ：按接口返回顺序平铺展示（分级字段由后端自带），不做排序/合并/映射
  const wxStatResult = useMemo(
    () =>
      buildWxStat({
        wxStatData,
        beginningBalance,
    ...
    [wxStatData, beginningBalance, endingBalance, currentYearMonth, currentShopName],
  );
  const wxSummary = wxStatResult.summary;
  const wxStatRows = wxStatResult;
    ...
    <>
      ...
      >
        <style>{`
          ...
          .stat-table-small tr.wx-stat-summary-row > td {
            background: #fafafa !important;
          }
        `}</style>

        {/* 微信余额对账（与抖音 / 共享 Base 的站内外推广费统计列一致） */}
        <div style={{ marginBottom: 8, fontSize: 14, fontWeight: 500 }}>微信余额对账</div>
        <Table
          columns={[
            {
              title: '账单月份',
          ...
          dataSource={statModalVisible ? [wxSummary] : []}
          rowKey="billMonth"
          size="small"
              ...
              <ul style={{ margin: 0, paddingLeft: 18 }}>
                ...
                <li style={{ marginBottom: 8 }}>
                  ...
                  <div style={{ marginLeft: 16 }}>
                    <div>本页用于查看微信店铺每月费用与余额，共两张表：<strong>微信余额对账</strong>（顶部）与<strong>微信费用统计</strong>（明细）。下方列明各项口径，供对账与核对。</div>
                  </div>
                </li>

                {/* 一、微信费用统计（明细表） */}
                <li style={{ marginBottom: 8 }}>
                  <strong>一、微信费用统计（明细表）</strong>
                  <div style={{ marginLeft: 16 }}>
        ...
        <Table
          columns={[
            {
              ...
              render: (text: string, _record: any, index: number) => {
                const span = wxStatRows.expCatSpan[index] ?? 1;
                if (span === 0) {
            ...
            },
            {
              ...
              width: 100,
              render: (text: string, _record: any, index: number) => {
                const span = wxStatRows.catSpan[index] ?? 1;
                if (span === 0) {
                  return { children: null, props: { rowSpan: 0 } };
                }
                return {
          ...
          dataSource={statModalVisible ? wxStatRows.rows : []}
          ...
          scroll={{ x: 620 }}
          summary={
            wxStatRows.rows.length > 0 && wxStatRows.totalRow
              ? () => (
                  <Table.Summary.Row className="wx-stat-summary-row">
                    <Table.Summary.Cell index={0} colSpan={4}>
                      <span style={{ fontSize: 12, fontWeight: 'bold' }}>
                        {wxStatRows.totalRow!.businessType || '总计'}
                      </span>
                    </Table.Summary.Cell>
                    <Table.Summary.Cell index={4} align="right">
                      <span
                        style={{
                          fontSize: 12,
                          fontWeight: 'bold',
                          color: wxSummary.total >= 0 ? '#389e0d' : '#cf1322',
                        }}
                      >
                        {(wxSummary.total >= 0 ? '+' : '') + wxSummary.total.toFixed(2)}
                      </span>
                    </Table.Summary.Cell>
                  </Table.Summary.Row>
                )
...
export default WxExtendCostBase;
