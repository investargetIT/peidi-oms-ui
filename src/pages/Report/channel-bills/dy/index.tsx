...
const DyBillPanel: React.FC = () => {
  ...
  const fetchBill = async (params: Partial<FinanceZfbBillInfoPageReq> = {}) => {
    ...
    try {
      ...
      console.error('查询抖音账单失败:', error);
      message.error('查询抖音账单失败');
    } finally {
  ...
  // 抖音目前暂不开放生成账单功能，改为支持手动上传账单
  const fetchConfigList = async () => {
    setConfigLoading(true);
    try {
      const res = await ManagementReportApi.getBillConfigList({ platform: '抖音' });
      if (res.code === 200) {
        // 过滤掉已删除的
        ...
        <Alert
          ...
          description={
            <div style={{ fontSize: 12, lineHeight: 1.7 }}>
              ...
              2. 请先在【账单配置】中维护该店铺的抖音账单配置（accessToken、appId 等）<br />
