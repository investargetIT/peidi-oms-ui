...
const KsBillPanel: React.FC = () => {
  ...
  const fetchBill = async (params: Partial<FinanceZfbBillInfoPageReq> = {}) => {
    ...
    try {
      ...
      console.error('查询快手账单失败:', error);
      message.error('查询快手账单失败');
    } finally {
  ...
  const fetchConfigList = async () => {
    ...
    try {
      const res = await ManagementReportApi.getBillConfigList({ platform: '快手' });
      if (res.code === 200) {
  ...
  const handleUpload = async () => {
    ...
    // 3. 异步执行上传（不 await 阻塞）
    ManagementReportApi.uploadKsBill({
      billDate: billDateStr,
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
            ...
            <Select
              ...
              placeholder={configLoading ? '加载中...' : '请选择账单配置'}
