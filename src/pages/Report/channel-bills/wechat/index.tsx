...
import ManagementReportApi, {
  ...
  type FinanceLedgerBillUploadVo,
...
const WechatBillPanel: React.FC = () => {
  ...
  const handleUpload = async () => {
    ...
    ManagementReportApi.uploadWxBill({
      ...
      .then((res) => {
        const result: FinanceLedgerBillUploadVo =
          (res.data as FinanceLedgerBillUploadVo) || ({} as FinanceLedgerBillUploadVo);
        if (res.code === 200 || res.success) {
          updateTask(taskId, { status: 'success', finishedAt: Date.now(), result });
